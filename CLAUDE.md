...
- **Dark mode is night, not inversion.** `--paper` and `--ink` NEVER flip. `.dark` redefines only the ground-following tokens — `--background`, `--text`, `--accent`, the alpha neutrals (`--ink-soft`, `--ink-mute`, `--rule`, `--rule-strong`), and the ground tints `--paper-2/3`. Panel faces stay lit paper with dark ink keylines in **both** themes (comic ink stays black in a night scene; an ink keyline on a dark face is ~1.2:1 and the outline dissolves). Long-form prose lives on the ground, not in panels, so it flips and night reading stays comfortable. `--accent` is red in light, yellow in dark; `::selection` is yellow with ink text in both themes; the "Current" stamp (`.exp-stamp`) is red-on-paper by day, yellow-on-ink at night.
...
- **Primitives:** `.xsheet` (the grid), `.xcell` (a slot, carries the side faces and the animated `--d` height), `.xcell-face` (the lit paper face with the 2px ink keyline).
...
- **Geometry.** The footprint never leaves its grid slot; the face lifts up-left by `translate(-d, -d)`. Side faces are skewed pseudo-element strips *inside* the slot on `.xcell`: `::before` is the right face (accent — red by day, yellow at night), `::after` is the bottom face (`--blue`, both themes). They hang off `.xcell`, not the face, precisely so they keep the flipping night accent.
...
- **Faces are daylight objects.** `.xcell-face` re-anchors the ground tokens *and* `--accent` to their light-theme values — face-interior accents are always red (yellow-on-paper is ~1.4:1). Caveat: `--muted` and `--sa-off-white` are `:root`-substituted chains the re-anchoring cannot reach — on faces use `--ink-mute`/`--paper-3` directly.
...
`components/ComicPanel.tsx` and the `.comic` / `.panel` / `.affiliations-strip` CSS still exist and are used by `/about` and `/projects`. After the token rewrite the panels render **paper-fixed** — lit paper faces with ink keylines even at night — so they sit consistently beside the extruded system, but don't introduce comic panels into new pages; they're carried forward only until those pages are individually rewritten. The old editorial card hover grammar and the `.rec-grid` card CSS are gone — Recommendations now uses the extruded sheet, and new grids/cards should too. The `.panel-link-hover` class is similarly legacy; new list-row hovers should use the editorial pattern below.
