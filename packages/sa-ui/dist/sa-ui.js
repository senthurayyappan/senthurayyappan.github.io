...
function getDuration(value, fallback) {
  ...
  return value.trim().endsWith("s") && !value.trim().endsWith("ms") ? duration * 1e3 : duration;
}
function getMark(element) {
  const mark = element.dataset.saMark;
  return mark === "underline" || mark === "circle" || mark === "highlight" ? mark : void 0;
}
...
function getElement(target) {
  ...
  const element = target.closest(".sa-mark[data-sa-mark]");
...
function enhance(root = document) {
  ...
  const show = (element) => {
    ...
    const activeMark = { annotation, frames: [] };
  ...
  const getInteractionTarget = (target) => {
    ...
    return element && getTrigger(element) === "interaction" ? element : void 0;
  };
